...
`AGENTS.md` at the repo root is the **source of truth** for repo-level AI/agent
guidance. `GEMINI.md` and `CLAUDE.md` at the repo root, and
`.github/copilot-instructions.md`, are all symlinks to this file — edit
`AGENTS.md`, not the symlinks.
...
- **`.agents/` is the canonical AI harness directory** — the single source of
  ...
  `.claude/` is a real directory at the repo root with selective symlinks into
  `.agents/` (commands → workflows, agents → agents, settings → settings.json).
  `.gemini/` is a full symlink to `.agents/`. Subprojects keep their own
  `.agents/` for decision logs but carry no per-tool symlinks.
  (`.vscode/` is **not** part of this alias tree — it's the committed VS Code
  dev config; see below.)
...
- **Doc taxonomy is fixed:** `CHANGELOG.md` = *what* changed (HA renders it
  ...
  one-paragraph blurb, `.agents/memory/` = project context notes (shared, but
  gitignored in subprojects — never link to `.agents/` from any shipped doc).
