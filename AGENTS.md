...
| `ship-pr.md` | branch → preflight → commit → push → PR → merge |
...
- **Stage gating:** `stage: experimental` in `config.yaml` = shown as experimental in the HA store; independently, gitignore controls whether the directory is tracked. Gitignored = local/dev only. To register a new app: add to `.gitignore` allowlist, release-please config, labels, labeler, issue templates.
